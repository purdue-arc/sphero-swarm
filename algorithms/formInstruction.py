...
def nextVectorDirection(actual, next):
    ...
    theta = np.arccos(d/ (np.linalg.norm(nextv) * np.linalg.norm(actualv))) # figure out what to do when actualv is 0?
...
'''
Given the actual coordinates and the intended coordinates calculate the distance needed to travel
'''
